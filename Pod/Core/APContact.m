...
- (void)mergeWith:(APContact *)contact {
    ...
    self.phones = [self.phones arrayByAddingObjectsFromArray:contact.phones];
    self.phonesWithLabels = [self.phonesWithLabels arrayByAddingObjectsFromArray:contact.phonesWithLabels];
    self.emails = [self.emails arrayByAddingObjectsFromArray:contact.emails];
    self.addresses = [self.addresses arrayByAddingObjectsFromArray:contact.addresses];
    self.socialProfiles = [self.socialProfiles arrayByAddingObjectsFromArray:contact.socialProfiles];
    self.linkedRecordIDs = [self.linkedRecordIDs arrayByAddingObjectsFromArray:contact.linkedRecordIDs];
}
...
- (NSArray *)arrayOfPhonesWithLabelsFromRecord:(ABRecordRef)recordRef {
    ...
    [self enumerateMultiValueOfProperty:kABPersonPhoneProperty
                             fromRecord:recordRef
                              withBlock:^(ABMultiValueRef multiValue, NSUInteger index)
     {
         ...
         if (phone) {
             NSString *originalLabel = [self originalLabelFromMultiValue:multiValue
                                                                   index:index];
             NSString *localizedLabel = [self localizedLabelFromMultiValue:multiValue
                                                                     index:index];
